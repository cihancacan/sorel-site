    ...
    <>
      <section className="pt-40 pb-0 bg-sorel-cream px-6 md:px-12 lg:px-20">
        <div className="sorel-container">
          ...
          <h1 className="sorel-heading text-5xl md:text-7xl mb-4" style={{ letterSpacing: '-0.02em' }}>{t('contact.title')}</h1>
          <p className="text-sm text-sorel-silver font-light max-w-md">{t('contact.subtitle')}</p>
        </div>
      </section>
                ...
                {[
                  ...
                  <div key={i} className="flex items-start gap-5 py-4 border-b border-sorel-black/5 last:border-0">
                    ...
                    <div>
                      ...
                      {item.href ? (
                        <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noopener noreferrer' : undefined} className="text-sm text-sorel-graphite hover:text-sorel-black transition-colors font-light">
                          {item.value}
                        </a>
                      ) : (
            ...
            <div className="lg:col-span-3">
              {submitted ? (
                <div className="py-20 text-center">
                  ...
                  <p className="font-display text-2xl font-light">{t('contact.form.success')}</p>
                </div>
              ) : (
                <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-7">
                  ...
                  <div>
                    ...
                    <textarea className={`${inputClass} resize-none`} rows={6} required />
                  </div>
